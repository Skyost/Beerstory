          ...
          subtitle: Column(
            ...
            children: [
              Padding(
                padding: const EdgeInsets.only(bottom: kSpace),
                child: Text(
                  translations.error.widget.subtitle(error: widget.error),
                ),
              ),
              if (widget.stackTrace != null) ...[
                FSwitch(
                  label: Text(
                    expanded ? translations.error.widget.button.hideTrace : translations.error.widget.button.showTrace,
                  ),
                  value: expanded,
                  onChange: toggleStackTrace,
                  style: (style) => style.copyWith(
                    childPadding: EdgeInsets.only(
                      right: style.childPadding.horizontal / 2,
                    ),
                    trackColor: FWidgetStateMap<Color>({
                      WidgetState.selected: context.theme.colors.error,
                      WidgetState.any: context.theme.colors.error.withValues(
                        alpha: 0.25,
                      ),
                    }),
                    labelTextStyle: FWidgetStateMap<TextStyle>.all(
